...
2. For a PDF, the extractor renders page 1, measures paper size, margins, dominant font, line height and heading colour, and proposes header/footer bands. Everything it returns has passed an occlusion check: content painted over by a later opaque shape is never reused, so a document "redacted" that way cannot be reconstructed through the import.
3. `POST /api/ingest/:id/fragment` either crops one PDF region (real SVG paths when the source is vector, a 4× PNG otherwise) or copies one composed DOCX band. It files the result in `backend/templates/assets`, which `typstCompile` already copies next to every compile, so the fragment is referenced by a bare filename, the only form Typst's path sandbox accepts.
4. `POST /api/ingest/:id/template` creates the template in Docs. The PDF request keeps the manual header/footer rectangles. The DOCX request selects header and footer in one operation, keeps `all` or `first`/`except-first` scopes, reserves their rendered heights in the margins, and maps recognized Word pagination to dynamic Typst numbering.
...
- `.docx` import deliberately handles only the first section. Multiple sections and distinct even-page bands are reported as warnings; the normal odd-page variant is retained for following pages because `LayoutConfig` has no even/odd scope.
