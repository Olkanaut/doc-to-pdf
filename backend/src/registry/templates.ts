...
const DATA_DIR =
  process.env.DOTS_DATA_DIR ?? path.resolve(__dirname, "../../data/templates");

/**
 * template appliqué par défaut : `{ "templateId": "…" }`, à côté des dossiers de
...
const SEEDS: { id: string; name: string; description: string; file: string }[] =
  [
    ...
    },
    {
      ...
      name: "Ministère",
      description:
        "En-tête avec logo et bandeau bleu, pagination en pied de page.",
      file: "ministere.typ",
    },
    {
      id: "collectivite",
    ...
    },
    {
      ...
      description:
        "Bloc Marianne officiel et nom du ministère en tête, filets bleu État.",
    ...
    },
    {
      ...
      name: "Ville de Paris",
      description:
        "Logo officiel de la Ville de Paris centré en tête, comme les arrêtés publiés.",
...
async function writeTemplateDir(
  id: string,
  meta: StoredMeta,
  source: string,
): Promise<void> {
...
async function regenerateThumbnail(id: string, source: string): Promise<void> {
  try {
    ...
    console.error(
      `[templates] thumbnail generation failed for "${id}": ${reason}`,
    );
  }
}
      ...
      .map(async (e) => {
        try {
          const raw = await readFile(
            path.join(templateDir(e.name), "meta.json"),
            "utf8",
          );
