...
// Référence : le gabarit de départ rendu sur la même fixture.
const ref = { gere: await mesurer(BASES.gere, "ref-gere"), libre: await mesurer(BASES.libre, "ref-libre") };
for (const f of readdirSync(`${DIR}/${OUT}`).sort()) {
  ...
  const collateral = c.allowed ? changed.filter((p) => !c.allowed!.some((a) => p === a || p.startsWith(`${a}.`))) : [];
  const verdict = r.ok
    ? c.check({ source: src, layout: after!.layout, before: before.layout, ok: true })
  ...
  const borne = brut
    ? Object.entries(flat(brut)).filter(([k, v]) => JSON.stringify(flat(sanitizeLayout(brut))[k]) !== JSON.stringify(v)).map(([k]) => k)
    : [];
  const m = r.source ? await mesurer(r.source, `${OUT}-${r.id}-${r.rep}`) : { ok: false, encre: -1, mots: -1, pages: -1 };
  const base = ref[c.base as "gere" | "libre"];
  rows.push({
    id: r.id, rep: r.rep, base: c.base, ok: r.ok, ms: r.ms,
    compiles: r.check?.ok === true,
    includeCount: (src.match(/#include\s+"body\.typ"/g) ?? []).length,
    blockKept: before.managed ? src.includes("// dots:layout begin") : !src.includes("// dots:layout"),
    coherent, intention: verdict === null, pourquoi: verdict, changed, collateral,
    /** Lignes posées après le bloc géré : l'échappatoire que le patch demande d'utiliser. */
    apresBloc: src.includes("// dots:layout end")
      ? src.split("// dots:layout end")[1].split('#include "body.typ"')[0].split("\n").filter((l) => l.trim()).length
      : 0,
    borne,
    encre: m.encre, mots: m.mots, pages: m.pages,
    /** Rapport à l'encre du gabarit de départ : < 0,5 = la page s'est vidée. */
    encreRatio: base.encre > 0 && m.encre >= 0 ? Math.round((m.encre / base.encre) * 100) / 100 : null,
...
console.log(`${OUT} — compile ${pc((r) => r.compiles)} · intention ${pc((r) => r.intention)} · bloc préservé ${pc((r) => r.blockKept)} · cohérent ${coh.filter((r) => r.coherent).length}/${coh.length} · sans collatéral ${pc((r) => r.collateral.length === 0)} · réponses avec surcharge hors bloc ${pc((r) => r.apresBloc > 0)}`);
for (const r of rows.filter((r) => r.borne.length))
  console.log(`   ⌦ ${r.id}#${r.rep} — réécrit en silence par sanitizeLayout : ${r.borne.join(", ")}`);
for (const r of rows.filter((r) => r.encreRatio !== null && (r.encreRatio < 0.5 || r.encreRatio > 2 || r.pagesDelta !== 0)))
  console.log(`   ⚠ ${r.id}#${r.rep} — encre ${Math.round((r.encreRatio ?? 0) * 100)} % du gabarit de départ, ${r.mots} mots, ${r.pages} page(s) (${(r.pagesDelta ?? 0) >= 0 ? "+" : ""}${r.pagesDelta})`);
for (const r of rows.filter((r) => !r.intention || r.coherent === false || r.collateral.length))
  console.log(`   ${r.id}#${r.rep}${!r.intention ? ` — intention: ${r.pourquoi}` : ""}${r.coherent === false ? " — incohérent" : ""}${r.collateral.length ? ` — non demandé: ${r.collateral.join(", ")}` : ""}`);
