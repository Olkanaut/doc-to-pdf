...
export function ComposePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchTemplates(), fetchDefaultTemplate().catch(() => null)])
      .then(([list, def]) => {
        ...
        const initial =
          wanted && list.some((template) => template.id === wanted)
            ? wanted
            : (def?.id ?? list[0]?.id ?? "");
        setTemplateId(initial);
      })
      .catch((reason: Error) => setError({ error: reason.message }));
  }, [searchParams]);
  ...
  useEffect(() => {
    ...
    fetchDocumentContent(documentId)
      ...
      .catch((reason: Error) => {
        if (!cancelled) setDocumentState({ status: "error", message: reason.message });
      });
        ...
        <section className="compose-center">
          <div className="compose-bar">
            {documentState.status === "loading" && (
              <span className="dots-muted">Chargement du document...</span>
            )}
            <a
          ...
          <div className="compose-preview">
            ...
            {documentState.status === "loaded" && (
              <div className="dots-notice compose-notice" role="status">
                Le contenu Docs est chargé. Le rendu PDF depuis ce contenu sera branché dans
                la prochaine étape.
              </div>
            )}
