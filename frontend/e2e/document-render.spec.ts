...
test("renders Docs content with the selected database template", async ({
  page,
}) => {
  ...
  await page.route(
    ...
    async (route) => {
      ...
      if (templateMatch) {
        ...
        return template
          ? route.fulfill({
              json: { ...template, source: '#include "body.typ"' },
            })
          : route.fulfill({
              status: 404,
  ...
  await expect(
    page.getByRole("group", { name: "Aperçu du PDF" }),
  ).toBeVisible();

  await page
    .getByRole("button", { name: "Informations sur le document" })
    .click();
