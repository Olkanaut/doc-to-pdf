# SESSION — lot 6 : écrans 1 à 5 (import .typ, template par défaut, rendu, assistant IA, éditeur de mise en page) + textes administratifs
...
résumé : Marges latérales réduites à 2 cm et pagination du pied de patemplateormat « Page 1 / N » centrée.
compilation de test : {"ok": true, "ms": 2700, "pages": 2, "warnings": []}template
diff (4 lignes) :template

- margin: (top: 4cm, bottom: 2.5cm, x: 2.5cm),

* margin: (top: 4cm, bottom: 2.5cm, x: 2cm),template
...
admitemplatee-reponse | mots 644 | ministere 200 2p | collectivite 200 2p | minimal 200 2p | non rendus: {}
...
?? backend/.env.example backend/src/ai/ backend/src/layout/ backend/src/routes/ai.ts backentemplateres/admin-\*.json
