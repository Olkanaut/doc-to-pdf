...
- Fichier : `backend/fixtures/reel-etat-circulaire-interieur.json` — 73 blocs, 0 tableau(x), rendu 4 page(s) avec le gabarit `ministere`, HTTP 200, blocs ignorés `{}`
...
$ curl -s -D - -o verif-etat.pdf -H "Content-Type: application/json" -d '{"fixtureId":"reel-etat-circulaire-interieur","templateId":"ministere"}' http://localhost:4000/api/render | grep -iE "^HTTP|x-dots|content-type"
HTTP/1.1 200 OK
x-dots-block-count: 73
x-dots-unsupported-blocks: {}
