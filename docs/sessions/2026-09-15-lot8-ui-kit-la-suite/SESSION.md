...
- épreuve de l'agent vérificateur (backend partagé :4000, données de test de l'humain avec
  quatre gabarits homonymes « Nouveau gabarit ») : `4 failed / 16 passed` — assistant-ia ②
  (sélecteur cassé par la TextArea du kit, corrigé par K2), gabarits ⑤ ⑧ et rendu ⑰
...
- passage suivant : `1 failed / 18 passed`, garde du spec mise-en-page (« minimal contient
  déjà un bloc dots:layout ») laissée par le passage incohérent ; restauré par
...
défaut : Ministère
gabarits : ['Collectivité', 'Minimal', 'Ministère', 'Nouveau gabarit', 'Nouveau gabarit', 'Nouveau gabarit', 'Nouveau gabarit']
```
...
- La copie n'a pas les changements faits ensuite dans l'arbre principal : bouton
  « + Déduire un gabarit d'un PDF » (TemplatesListPage), `max_tokens`/délai du client IA,
...
5. Retour dans l'arbre principal : diff de `frontend/` entre les deux worktrees (21 fichiers
   ...
   le bouton « Déduire un gabarit d'un PDF ».

## Confidentialite
Données de test synthétiques seulement (gabarits de semis, fixtures d'exemple). Aucune donnée
...
✓ built in 302ms
```
Le bouton « + Déduire un gabarit d'un PDF » (ajouté après la copie) est re-posé sur la page Gabarits avec le
